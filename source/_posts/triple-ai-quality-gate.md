...
```
[原始代码] → [AI-1 档案员] → [事实锚点] → [AI-2 啄木鸟] → [审查报告] → [AI-3 铁匠] → [最终方案]
```
...
[原始项目代码]
       ...
       v   AI-1 一次性探测扫描
┌─────────────────┐
│   AI-1 档案员    │  只收集事实，不分析不判断
│  (Archivist)     │
└────────┬────────┘
         |  只输出结构化事实陈述
         v
┌─────────────────┐
│ project_context  │  事实锚点文件（含完整性声明）
│     .txt         │
└────────┬────────┘
         |
         +--- 操作者快速浏览 → 完整性声明确认
         |    （如发现遗漏 → 手动补充至文件中）
         |
         v  投喂 project_context.txt + 核心代码片段
┌─────────────────┐
│   AI-2 啄木鸟    │  只做减法：找错、删减、警告
│  (Woodpecker)    │
└────────┬────────┘
         |  只输出结构化表格（含上下文缺口）
         v
...
└────────┬────────┘
         |
         v  投喂 project_context.txt + audit_report_v1.md
┌─────────────────┐
│   AI-3 铁匠      │  验证、修正、落地
│  (Blacksmith)    │
└────────┬────────┘
